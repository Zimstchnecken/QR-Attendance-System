
# QR Attendance Frontend (Expo)

- SDK 54 ginamit ko, bale idownload niyo dito https://expo.dev/go?sdkVersion=54&platform=android&device=true

## Requirements
- Node.js 
- Expo Go app nasa playstore siya, pag crim ka baka pumunta ka pa ng SM

## Install
- npm install
- npx expo start
- after ng expo start may malaking qr code iscan mo sa expo go mo
- kailangan parehas kayo ng wifi sa laptop/phone
- control c pag stop server

clear cache
- npx expo start -c


## Notes
- UI is frontend-only (no backend or database)
- QR scanner and email actions are placeholders
