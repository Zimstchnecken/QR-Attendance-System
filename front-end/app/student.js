...
const SCANNER_REMINDERS = [
  "Hold your phone steady and align the QR inside the frame.",
  "If offline, your check-in is queued and retried securely.",
];
...
export default function StudentScreen({
  ...
  const [scanFeedback, setScanFeedback] = useState({ type: "idle", message: "Ready to scan." });
  ...
  const loadHistory = useCallback(async () => {
    if (!studentId || !session?.accessToken) return;
    setHistoryLoading(true);
    try {
      const data = await fetchStudentAttendanceHistory(studentId, session.accessToken);
      const mapped = (data.recentRecords ?? []).map((r) => ({
        id: r.sessionId,
        className: r.className ?? "—",
        status: formatStatus(r.status),
        time: r.scannedAt ? formatTime(r.scannedAt) : "—",
  ...
  useEffect(() => {
    Animated.timing(screenAnim, { toValue: 1, duration: 280, useNativeDriver: true }).start();
    Animated.stagger(
      50,
      cardAnims.map((anim) => Animated.timing(anim, { toValue: 1, duration: 250, useNativeDriver: true }))
    ).start();
  ...
  const handleBarcodeScanned = ({ data }) => {
    if (!data || isScanLocked) return;
    setIsScanLocked(true);
    processScanPayload(data);
    setTimeout(() => setIsScanLocked(false), 1200);
  };

  const handleSimulateScan = () => {
    if (!activeQrSession || isScanLocked) return;
    setIsScanLocked(true);
    processScanPayload(activeQrSession.encoded);
    setTimeout(() => setIsScanLocked(false), 900);
  };

  const screenStyle = useMemo(
    () => ({
      opacity: screenAnim,
      transform: [{ translateY: screenAnim.interpolate({ inputRange: [0, 1], outputRange: [10, 0] }) }],
    }),
    [screenAnim]
  );
  ...
  const historyRows = useMemo(() => {
    ...
    return [...liveRows, ...apiHistory];
  }, [scanEvents, apiHistory]);

  const latestRecord = historyRows[0] ?? null;
    ...
    if (!permission.granted) {
      return (
        <View className="w-full items-center justify-center rounded-2xl border border-primary/20 bg-card px-4" style={styles.scannerFrame}>
          ...
          <TouchableOpacity activeOpacity={0.9} onPress={requestPermission} className="mt-4 rounded-2xl bg-primary px-4 py-3">
      ...
      return (
        <GlassCard className="mb-6 overflow-hidden" style={[styles.homeCard, cardStyle(cardAnims[0])]}>
          ...
          <View className="mb-4 flex-row items-start justify-between gap-3" style={isCompact ? styles.compactHeaderRow : null}>
            <View className="flex-1" style={isCompact ? styles.compactHeaderTitle : null}>
              ...
              <Text className="mt-2 text-sm text-textSecondary font-sans">
                {studentName} — ready to check in. Open the scanner when class starts.
              </Text>
            </View>
          ...
          <View className="rounded-2xl border border-success/20 bg-success/10 p-4">
            {latestRecord ? (
              <>
                <Text className="text-sm text-textSecondary font-sans">Last class: {latestRecord.className}</Text>
                <Text className="mt-1 text-sm text-textSecondary font-sans">
                  {latestRecord.date}{latestRecord.time ? ` · ${latestRecord.time}` : ""}
                </Text>
                <Text className="mt-2 text-sm font-semibold text-success font-sans">
                  {scanFeedback.type !== "idle" ? scanFeedback.message : `Status: ${latestRecord.status}`}
                </Text>
              </>
            ) : (
              <Text className="text-sm text-textSecondary font-sans">
                {scanFeedback.type !== "idle" ? scanFeedback.message : "No attendance records yet."}
              </Text>
            )}
          </View>
      ...
      return (
        <GlassCard className="mb-6 overflow-hidden" style={[styles.scannerCard, cardStyle(cardAnims[1])]}>
          ...
          <View className="mb-3 rounded-2xl border border-primary/20 bg-primary/10 p-3">
            <Text className="text-xs font-semibold uppercase tracking-widest text-textSecondary font-sans">Active Session</Text>
          ...
          <View className="mt-4 rounded-2xl border border-primary/20 bg-primary/10 p-4">
            {SCANNER_REMINDERS.map((tip, index) => (
              <View
                key={`${index}-${tip.slice(0, 10)}`}
                className={index === SCANNER_REMINDERS.length - 1 ? "flex-row gap-2" : "mb-2 flex-row gap-2"}
              >
          ...
          <TouchableOpacity
            ...
            className={`mt-4 w-full rounded-2xl px-4 py-4 ${!activeQrSession || isScanLocked ? "bg-surface" : "bg-primary"}`}
            style={styles.actionButton}
          >
            <View className="flex-row items-center justify-center gap-2">
              <CircleCheck size={18} color={!activeQrSession || isScanLocked ? theme.colors.textSecondary : "#FFFFFF"} />
              <Text className={`text-center text-base font-semibold font-sans ${!activeQrSession || isScanLocked ? "text-textSecondary" : "text-white"}`}>
                Mark Attendance
              </Text>
            </View>
          </TouchableOpacity>
          <Animated.View
            className={`mt-3 flex-row items-center gap-2 rounded-xl px-3 py-3 ${
              scanFeedback.type === "success" ? "bg-success/10" : scanFeedback.type === "error" ? "bg-danger/10" : "bg-primary/10"
            }`}
            style={{
              opacity: successAnim,
          ...
          >
            <CircleCheck
              size={18}
              color={
                scanFeedback.type === "success" ? theme.colors.success
                : scanFeedback.type === "error" ? theme.colors.danger
                : theme.colors.primary
              }
            />
            <Text className={`text-sm font-semibold font-sans ${
              scanFeedback.type === "success" ? "text-success"
              : scanFeedback.type === "error" ? "text-danger"
              : "text-primary"
            }`}>
      ...
      <GlassCard className="mb-6 overflow-hidden" style={[styles.historyCard, cardStyle(cardAnims[2])]}>
        ...
        {historyLoading && historyRows.length === 0 && (
          <View className="my-8 items-center">
            ...
            <Text className="mt-3 text-sm text-textSecondary font-sans">Loading history…</Text>
          </View>
        )}
        {!historyLoading && historyRows.length === 0 && (
          <View className="my-8 items-center">
        ...
        {historyRows.map((row) => (
          <View key={row.id} className="mb-3 rounded-2xl border border-border bg-card p-4" style={styles.historyRow}>
            <View className="flex-row items-center justify-between" style={isCompact ? styles.compactHistoryRowTop : null}>
              ...
              <View className={`rounded-full px-2 py-1 ${row.status === "Late" || row.status === "Absent" ? "bg-danger/10" : "bg-success/10"}`}>
                <Text className={`text-xs font-semibold font-sans ${row.status === "Late" || row.status === "Absent" ? "text-danger" : "text-success"}`}>
                  {row.status}
                </Text>
              </View>
            </View>
            {row.date ? <Text className="mt-2 text-sm text-textSecondary font-sans">Date: {row.date}</Text> : null}
            {row.time ? <Text className="text-sm text-textSecondary font-sans">Time: {row.time}</Text> : null}
  ...
  return (
    <SafeAreaView className="flex-1 bg-background">
      <ScreenBackground />
      <Animated.ScrollView style={[styles.scrollView, screenStyle]} contentContainerStyle={[styles.scrollContent, scrollPadding]}>
...
const styles = StyleSheet.create({
  ...
  bannerCard: { backgroundColor: theme.colors.primary, ...theme.shadow.card },
  bannerPaddingCompact: { paddingHorizontal: theme.spacing.md, paddingTop: theme.spacing.lg, paddingBottom: theme.spacing.md },
  compactHeaderRow: { alignItems: "stretch" },
  compactHeaderTitle: { flexBasis: "100%" },
  compactPageHeader: { alignItems: "stretch" },
  compactPageHeaderText: { flexBasis: "100%", paddingRight: 0 },
  compactTitleBlock: { flexBasis: "100%", paddingRight: 0 },
  compactTitleText: { fontSize: 28, lineHeight: 32 },
  compactBadge: { alignSelf: "flex-start" },
  compactLogoutButton: { alignSelf: "stretch", alignItems: "center" },
  compactChipWrap: { rowGap: theme.spacing.xs },
  actionButton: { minHeight: 48, justifyContent: "center" },
  stackActionRow: { flexDirection: "column" },
  fullWidthButton: { width: "100%" },
  bannerGlowTop: { position: "absolute", right: -24, top: -42, width: 164, height: 164, borderRadius: 999, backgroundColor: "rgba(255,255,255,0.18)" },
  bannerGlowBottom: { position: "absolute", left: -54, bottom: -64, width: 178, height: 178, borderRadius: 999, backgroundColor: "rgba(255,255,255,0.14)" },
  homeCard: { borderColor: "rgba(15,118,110,0.22)" },
  homeGlow: { position: "absolute", right: -30, top: -34, width: 114, height: 114, borderRadius: 999, backgroundColor: "rgba(15,118,110,0.1)" },
  scannerCard: { borderColor: "rgba(15,118,110,0.22)" },
  scannerGlow: { position: "absolute", right: -34, top: -36, width: 124, height: 124, borderRadius: 999, backgroundColor: "rgba(15,118,110,0.1)" },
  scannerFrame: { height: 260, borderRadius: theme.radius.card, backgroundColor: theme.colors.card },
  cameraView: { flex: 1 },
  cameraOverlay: { ...StyleSheet.absoluteFillObject, alignItems: "center", justifyContent: "center" },
  scanWindow: { width: "72%", aspectRatio: 1, borderWidth: 2, borderColor: "rgba(255,255,255,0.92)", borderRadius: 22, backgroundColor: "rgba(255,255,255,0.08)" },
  historyCard: { borderColor: "rgba(22,163,74,0.22)" },
  historyGlow: { position: "absolute", right: -30, top: -34, width: 118, height: 118, borderRadius: 999, backgroundColor: "rgba(22,163,74,0.1)" },
  historyRow: { borderColor: "rgba(229,231,235,0.9)" },
  compactHistoryRowTop: { flexDirection: "column", alignItems: "flex-start", gap: 6 },
});
